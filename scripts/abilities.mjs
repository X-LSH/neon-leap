...
/**
 * ★ 朴素解法的门槛检查（这一条是被玩家实测失败逼出来的）。
 *
 * 「跳上去 → 抓住墙 → 往上爬」是**玩家面对高墙时的第一反应**。
 * 它必须能爬过设计中最高的单面墙 —— 否则玩家会觉得「这游戏根本过不去」，
 * 而不会去猜「要先爬到力竭、再墙跳、再上冲」那套反直觉的技巧循环。
 *
 * 技巧循环（下一条）能到 19 格毫无意义，如果朴素解法连 4 格都上不去。
 */
{
  const naive = probeRise({
    world: WALL, spawn: SPAWN_AT_WALL,
    inputFor: (i) => mix({
      moveX: 1, moveY: -1,
      jump: { held: i < HOLD_FRAMES, pressed: i === 0 },
  ...
  record('朴素养法能上的墙高', cells(naive), '必须 ≥ 5 格，否则普通玩家上不去');
}
