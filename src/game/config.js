...
export const CFG = {
  ...
  wallJumpVx: 175,      // u/s  墙跳水平初速（自动背离墙面）
                        //      从 215 降下来：原值让玩家墙跳后要飞 1.7 格才回得来，
                        //      而墙跳只升 0.96 格 —— 净上升≈0，墙跳形同虚设
  wallJumpVy: -215,     // u/s  墙跳垂直初速
  wallJumpLock: 0.12,   // s    墙跳后水平输入锁定（防止立刻贴回）

  // ── 抓墙攀爬
  // ⚠️ climbStamina 从 1.10 提到 1.90 是**被玩家实测逼出来的**：
  //    1.10 时「跳 + 抓墙爬」只能上 3.89 格，而测试场的高墙是 4 格 ——
  //    玩家最直觉的解法差 0.11 格爬不上去，于是得出「这游戏过不去」的结论。
  //    技巧循环（爬到力竭 → 墙跳 → 上冲）能上 19 格毫无意义，
  //    如果第一反应的做法连 4 格都上不去。
  climbStamina: 2.60,   // s    抓墙耐力（限制**单次**攀爬窗口；墙跳会立刻回满，见 player.js）
  climbUpSpeed: 45,     // u/s  向上攀爬速度
  climbDownSpeed: 80,   // u/s  向下攀爬速度

  // ── 翻越（mantle）
  // 玩家爬到墙沿时自动翻上去。**这不是一个可选的手感润色，而是抓墙动词的必需边界情形** ——
  // 没有它，玩家爬到墙顶那一刻墙消失、原地自由落体，而它恰好悬在墙沿外侧，
  // 没有任何水平速度能把玩家推上去（曾因此报废两关）。
  //
  // 之所以做成「自动」而不是「按上+跳」：抓墙时按跳已经绑定给了墙跳，
  // 再叠加手动翻越会让同一个手势在相近状态下产生两种结果 —— 那是玩家困惑的源头。
  mantleVy: -205,       // u/s  翻越的向上冲量（需把整个身高提到墙顶之上，约 12 单位）
  mantleVx: 135,        // u/s  翻越的朝墙冲量（在上升期内横向移动到墙顶上方）
  mantleTime: 0.22,     // s    翻越窗口：期间撞墙不清零水平速度，否则永远移不过去
